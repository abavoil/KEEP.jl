...
u0s = [ss; repeat([0, dτ_sign(0.1), 0], inner=(1, size(ss, 2)))]
u0s
...
Ts, y_Ts = invert(x=[compute_limit_cycle_2callbacks(SA[q_dq..., 0], vbp, nb_cycles=5) for q_dq in ProgressBar(q_dqs)])
...
PM4.integrate(SA[q_dqs[1]..., 0], -0.1, vbp)
