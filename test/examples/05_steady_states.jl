...
τ_init = 0.5
ss = SS.aligned_and_opposite_steady_states(τ_init, vbp; tol=tol)
@test all(norm.(SS.ddq_partial.(ss, Ref(vbp))) .< tol)
...
complete_steady_states = combinedims(flatten(SS.aligned_and_opposite_steady_states.(τs, Ref(vbp); tol=tol)))

steady_states = reduce_expand_reduce(complete_steady_states, 10tol)

# Verify that they are all equilibriums
@test all(norm.(SS.ddq_partial.(eachcol(steady_states), Ref(vbp))) .< tol)

# Verify that these equilibriums are all different
@test all(compute_adjacency(steady_states, tol) - I .== 0)

# sort steady states by φ
sort_inds_φ = sortperm([SS.aligned_α(τ, vbp) for (_, τ) in eachcol(steady_states)])
...
complete_steady_states_QM = combinedims(SS.steady_state.(αs, τs, Ref(vbp); tol=tol))
