...
# Keywords for Bounds
- `min_tf`: Minimum final time (period).
- `max_tf`: Maximum final time (period).
- `min_power`: Minimum power.
- `max_power`: Maximum power.
- `min_alpha`: Minimum alpha angle.
- `max_alpha`: Maximum alpha angle.
- `min_d_alpha`: Minimum alpha angular velocity.
- `max_d_alpha`: Maximum alpha angular velocity.
- `min_abs_d_tau`: Minimum absolute tau angular velocity.
- `max_abs_d_tau`: Maximum absolute tau angular velocity.
...
"""
function optimize(p, optim_para_syms, optim_para_lower, optim_para_upper;
    sense=+,
    tol=10DEFAULT_TOLERANCE,
    max_wall_time=120.,
    linear_solver="mumps",
    ...
    ## Setup
    # Optimization variables: α0, dα0, dτ0, tf, P, [parameters]...
    vbp = normalize_vbpara(vbp0)  # without dimensions
    
    # Initialization of a state on the limit cycle
    ...
    ## Box constraints
    # Arbitrary box for α, dα, dτ, tf, P
    min_d_tau_val, max_d_tau_val = minmax(sense(min_abs_d_tau), sense(max_abs_d_tau))
    lb = CA([min_alpha, min_d_alpha, min_d_tau_val, min_tf, min_power, lower_vbp[optim_para_syms]...], OPT_VARS_AXES)
    ub = CA([max_alpha, max_d_alpha, max_d_tau_val, max_tf, max_power, upper_vbp[optim_para_syms]...], OPT_VARS_AXES)

    # Box at ±10% of Initialization
    # ε = .1
    # lb = min.((1 - ε) * optim_vars, 1 / (1 - ε) * optim_vars)
    # ub = max.((1 + ε) * optim_vars, 1 / (1 + ε) * optim_vars)

    ## Cycle connection constraints
